...
- [ ] 6. `POST /api/memories` crashes with a 500 on a non-string `fact`, and measures the 300-char cap on untrimmed text.
- [ ] 7. `/api/metrics/charts` leaves aging and cash-flow sums unrounded while `/summary` and `/briefing` round — the same totals disagree at the cent.
...
- [ ] 10. `InvoiceDrawer`'s share errors render in the success colour — the same bug fixed in #96 for `KnowledgeCard`.
